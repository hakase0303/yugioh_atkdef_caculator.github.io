...
export function _a(base, effects) {
  ...
  const _ac = (e) => {
    if (!e.vary || e.vary.next === undefined) return false;
    const i = _q.indexOf(e);
    if (i < 0) return true;
    const next = _q[i + 1];
    return !next || e.vary.after - 1 < next.order;
  };
  ...
  const _af = (upto) => {
    for (const d of list) {
      ...
      fired.add(d.order);
      if (_v.has(d.order)) continue;
      const before = cur();
      if (d.vary.delta !== undefined && _f[d.cat].kind === "delta") {
        const held = _bg.find((x) => x.order === d.order);
        if (!held) continue;
        held.value += d.vary.delta;
      } else if (d.vary.next !== undefined && _f[d.cat].group === "current") {
        if (!_bf || _bf.order !== d.order) continue;
        _bf = { ..._bf, value: _c(cur(), { ...d, value: d.vary.next }, orig) };
      } else continue;
      push({ ...d, value: d.vary.next ?? d.value }, before, cur(), "개체수 변동");
    }
  };
  ...
  for (const e of list) {
    ...
    if (c.group === "original") {
      const before = cur();
      const src = _ac(e) ? { ...e, value: e.vary.next } : e;
      const b2 = orig;
...
export function _b(base, effects, other = [], otherBase = 0) {
  ...
  const _aj = (drop) => {
    const _H = effects
      ...
      .map(({ e, i }) =>
        e.vary && e.vary.next !== undefined ? { e: { ...e, value: e.vary.next }, i } : { e, i }
      )
      .map(({ e, i }) => {
        const c = _f[e.cat];
...
export function _aa(self, other, otherBase, selfBase = 0, depth = 3) {
  ...
  return self.map((e) => {
    ...
    return { ...e, op: "fixed", value, readsCurrent: true };
  });
}
...
.yc-var{grid-column:2/7;display:flex;flex-wrap:wrap;align-items:flex-end;gap:10px;
  margin:2px 0 4px;padding:0;font-size:11.5px;color:var(--muted);}

.yc-off{grid-column:7/-1;display:flex;flex-wrap:wrap;align-items:flex-end;gap:10px;
  margin:2px 0 4px;padding:0;font-size:11.5px;color:var(--muted);}
.yc-off .yc-sel{width:auto;min-width:110px;}
...
@container (max-width:560px){
  ...
  .yc-row[data-mode="1"][data-num="0"]{
    ...
    "n t f g g m"
    "n o o o o2 m";}
  .yc-row[data-x="1"][data-open="0"]{
    grid-template-areas:
    "n t f g g m1"
    "n x v o o m2";}
  .yc-row[data-x="1"][data-open="0"][data-num="0"]{
    grid-template-areas:
    "n t f g g m1"
    "n x o o o m2";}
  .yc-row[data-x="1"][data-open="0"][data-mode="1"]{
    grid-template-areas:
    "n t f g g m1"
    "n x v o o m2"
    "n o2 o2 o2 o2 o2";}
  .yc-row[data-x="1"][data-open="0"][data-mode="1"][data-num="0"]{
    grid-template-areas:
    "n t f g g m1"
    "n x o o o m2"
    "n o2 o2 o2 o2 o2";}
  .yc-row[data-x="1"][data-vary="1"][data-offrow="0"]{
    ...
    "n x y y y y"
    "n z v o o m2";}
  .yc-row[data-x="1"][data-vary="1"][data-offrow="0"][data-num="0"]{
    grid-template-areas:
    "n t f g g m1"
    "n x y y y y"
    "n z o o o m2";}
  .yc-row[data-x="1"][data-vary="1"][data-offrow="0"][data-mode="1"]{
    grid-template-areas:
    "n t f g g m1"
    "n x y y y y"
    "n z v o o2 m2";}
  .yc-row[data-x="1"][data-vary="1"][data-offrow="0"][data-mode="1"][data-num="0"]{
    grid-template-areas:
    "n t f g g m1"
    "n x y y y y"
    "n z o o o2 m2";}
  .yc-row[data-x="1"][data-vary="0"][data-offrow="1"]{
    grid-template-areas:
    "n t f g g m1"
    "n x v o o m2"
    "n z w w w w";}
  .yc-row[data-x="1"][data-vary="0"][data-offrow="1"][data-num="0"]{
    grid-template-areas:
    "n t f g g m1"
    "n x o o o m2"
    "n z w w w w";}
  .yc-row[data-x="1"][data-vary="0"][data-offrow="1"][data-mode="1"]{
    grid-template-areas:
    "n t f g g m1"
    "n x v o o2 m2"
    "n z w w w w";}
  .yc-row[data-x="1"][data-vary="0"][data-offrow="1"][data-mode="1"][data-num="0"]{
    grid-template-areas:
    "n t f g g m1"
    "n x o o o2 m2"
    "n z w w w w";}
  .yc-row[data-x="1"][data-vary="1"][data-offrow="1"]{
    ...
    "n z w w w w"
    "n v o o o m2";}
  .yc-row[data-x="1"][data-vary="1"][data-offrow="1"][data-num="0"]{
    grid-template-areas:
    "n t f g g m1"
    "n x y y y y"
    "n z w w w w"
    "n o o o o m2";}
  .yc-row[data-x="1"][data-vary="1"][data-offrow="1"][data-mode="1"]{
    grid-template-areas:
    "n t f g g m1"
    "n x y y y y"
    "n z w w w w"
    "n v o o o2 m2";}
  .yc-row[data-x="1"][data-vary="1"][data-offrow="1"][data-mode="1"][data-num="0"]{
    grid-template-areas:
    "n t f g g m1"
    "n x y y y y"
    "n z w w w w"
  ...
  .yc-row .yc-cv,.yc-row .yc-offchip{height:40px;align-self:center;font-size:10px;padding:0 2px;}
    ...
    const _Y = (list) =>
      list
        ...
        .map((e) => {
          ...
          const after = Number(e.after) || 1;
          return e.grp === "delta"
            ? { ...base, value: unit * c1, vary: { after, delta: unit * (c2 - c1) } }
            : { ...base, value: unit * c1, vary: { after, next: unit * c2 } };
        });
    ...
    return (
      ...
      >
        ...
        <select
          ...
          onChange={(ev) => {
            patch(s, e.id, {
              grp: ev.target.value, op: null, opBase: "", opMode: "",
            });
          }}
        >
          <option value="" disabled>선택</option>
        ...
        {canOff ? (
          <div className="yc-c-x yc-chips">
            ...
            <button
              ...
              onClick={() => patch(s, e.id, { off: !e.off })}
            >
              OFF
            </button>
          </div>
        ...
        {canVary && e.varying && (
          <div className="yc-var">
            ...
            <label className="yc-varf">
              ...
              <input className="yc-input" inputMode="numeric"
                placeholder={tight ? "이후개수" : "수치"} aria-label="이후개수"
                value={e.count2}
          ...
          <div className="yc-off">
            {(
              <label className="yc-varf yc-varf--off">
                {!tight && <span>무효·파괴되는 시점</span>}
                <select className="yc-sel" value={e.offAfter}
                  aria-label="무효·파괴되는 시점"
                  onChange={(ev) => patch(s, e.id, { offAfter: ev.target.value })}>
                  {lists[s].map((_, n) => (
                    <option key={n} value={n + 1}>
                      {tight ? `적용 순서 ${n + 1} 이후 무효` : n + 1}
                    </option>
                  ))}
                </select>
              </label>
