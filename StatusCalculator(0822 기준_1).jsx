...
const _d = (v) => Math.max(0, Math.round(v));
...
const _e = (eff) => {
  const o = [..._g.delta, ..._g.set].find((x) => x.id === eff.op);
...
export function _a(base, effects) {
  ...
  const _x = (startValue, items) => {
    ...
    for (const e of items) {
      ...
      if (c.kind === "set" && c.timing === "continuous" && _y.length) {
        v = _c(v, e, _G);
        if (e.op === "fixed") _G = v;
        trace.push({
        ...
        _z.forEach((p) => { if (!_y.includes(p)) _v.add(p.order); });
        for (const d of _y) {
          const b2 = v;
          v = _c(v, d, _G);
          trace.push({ kind: "step", cat: d.cat, label: _e(d), before: b2, value: v, note: "재계산" });
        }
        _z.push(e);
        continue;
      }
      ...
      if (c.kind === "set") {
        const erased = _z.length > 0;
        _z.forEach((p) => _v.add(p.order));
        ...
        if (c.timing === "continuous") _y.push(e);
  ...
  return { original, current, trace, _v, _w };
}
