...
export function _a(base, effects) {
  ...
  const _ag = (e) => {
    ...
    return _bu(item ? item.beneath : undefined);
  };
  ...
  for (const e of list) {
    ...
    if (c.kind === "delta") {
      ...
      for (const item of live) {
        ...
        const foe = _c2.includes(item.eff.op);
        if (!_p.includes(item.eff.op) && !foe) continue;
        const b3 = cur();
        anchor = { ...anchor, value: _c(orig, item.eff, orig),
                   order: item.eff.order };
        deltas = foe ? [] : deltas.filter((d) => d.timing === "continuous");
    ...
    const before = cur();
    const inWindow = (d) => !(d.timing === "continuous" && d.temp);
    const readAt = () => {
      let v = anchor ? anchor.value : orig;
      for (const d of deltas) if (inWindow(d)) v = _d(v + d.value);
      return v;
    };
...
export function _b(base, effects, other = [], otherBase = 0) {
  ...
  const without = (drop) => {
    const survivors = effects
      ...
      .filter(({ e, i }) => {
        ...
        if (!full._v.has(i)) return true;
        if (e.vary) return true;
        const by = full._ad.get(i);
        return !!by && drop(by);
      })
      .map(({ e, i }) =>
  ...
  const now = hasOff ? without((e) => !!e.off) : full;
  if (hasOff) {
    effects.forEach((e) => {
      if (!e.off) return;
      now.trace.push({
...
export function _aa(self, other, otherBase, selfBase = 0, depth = 3) {
  ...
  return self.map((e) => {
    ...
    const cut = otherSwaps[seen++] ?? other.length;
...
export default function StatusCalculator() {
  ...
  const results = useMemo(() => {
    ...
    const prepped = Object.fromEntries(
      STATS.map((s) => [s, prep(lists[s], Number(_c3[s]) || 0)])
    );
