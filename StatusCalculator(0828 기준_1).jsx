...
export function _a(base, effects) {
  ...
  for (const [i, e0] of _q.entries()) {
    const e = _ac(e0, i) ? { ...e0, value: e0.vary.next } : e0;
  ...
  const _x = (startValue, items) => {
    ...
    const _af = (upto) => {
      for (const d of items) {
        ...
        fired.add(d.order);
        if (locked.has(d.order) || _v.has(d.order)) continue;
        const b2 = v;
        if (d.vary.delta) {
          v = _d(v + d.vary.delta);
        } else if (d.vary.next !== undefined && d.vary.next !== d.value) {
          v = _c(v, { ...d, value: d.vary.next }, _G);
        } else {
          continue;
        }
        trace.push({
          kind: "step", cat: d.cat, label: _e({ ...d, value: d.vary.next ?? d.value }),
    ...
    const _ae = (upTo, by) => {
      const redoDelta =
        _f[by.cat].timing === "continuous" &&
        !READS_CURRENT.includes(by.op) &&
        !by.readsCurrent;
      if (!redoDelta) _ai.forEach((d) => locked.add(d.order));
      for (const d of _ai) {
        if (d.order >= upTo) continue;
        const now = _ag(d);
        if (_f[d.cat].kind === "delta") {
          if (!redoDelta) continue;
        } else if (_ah.get(d.order) === now) continue;
        const b2 = v;
        v = _c(v, d, _G);
        _ah.set(d.order, now);
        if (v === b2) continue;
    ...
    for (const e of items) {
      ...
      _ah.set(e.order, _ag(e));
      v = _c(v, e, _G);

      if (c.kind === "set") {
        const erased = _z.some((p) => !_ai.includes(p));
        _z.forEach((p) => {
          if (_ai.includes(p)) return;
          _v.add(p.order);
          _ad.set(p.order, e);
        });
        ...
        if (c.timing === "continuous") _ai.push(e);
        _ae(e.order, e);
      } else {
        if (c.timing === "continuous") _ai.push(e);
  ...
  const _D = _C
    ? _q.find((e) => e.order > _C.order && MULT.includes(e.op))
    : null;
...
.yc-chips{display:flex;gap:4px;}
.yc-chips .yc-chip{width:auto;padding:0 8px;}
...
@container (max-width:560px){
  ...
  .yc-row[data-x="1"]{grid-template-areas:"n t f g m1"
                                          "n x v o m2";}
  .yc-row[data-x="1"][data-num="0"]{grid-template-areas:"n t f g m1"
                                                        "n x o o m2";}
  
  .yc-row[data-x="1"][data-vary="1"]{grid-template-areas:"n t f g m1"
                                                         "n x y y y"
                                                         "n . v o m2";}
  .yc-row[data-x="1"][data-vary="1"][data-num="0"]{grid-template-areas:"n t f g m1"
                                                                       "n x y y y"
                                                                       "n . o o m2";}
  ...
  .yc-chips{flex-direction:column;gap:3px;height:100%;}
  .yc-chips .yc-chip{width:100%;padding:0;flex:1;height:auto;min-height:0;font-size:9px;}
  ...
  const _K = (s, e, i) => {
    ...
    return (
      <div
        ...
        data-num={_X ? "1" : "0"}
        data-x={canOff ? "1" : "0"}
        data-vary={(canVary && e.varying) || (canOff && e.off) ? "1" : "0"}
      ...
      >
        ...
        {canOff ? (
          <div className="yc-c-x yc-chips">
            {canVary && (
              <button
                className="yc-chip" aria-pressed={e.varying ? "true" : "false"}
            ...
            <button
              className="yc-chip" aria-pressed={e.off ? "true" : "false"}
        ...
        {((canVary && e.varying) || (canOff && e.off)) && (
