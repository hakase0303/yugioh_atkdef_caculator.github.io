...
function _c5(e, _v, _ad, drop) {
  if (drop(e)) return false;
  if (!_v.has(e._order ?? e.order)) return true;
...
export function _a(base, effects, { flushOff = true, until = Infinity } = {}) {
  ...
  const _bu = (beneath, _c7 = []) => {
    let v = beneath ?? orig;
    for (const d of _c7) v = _d(v + d.value);
    for (const d of deltas) v = _d(v + d.value);
    return v;
  };
  const _ag = (e) => {
      ...
      _bv(d).forEach((v, k) => {
        ...
        if (v.delta !== undefined && _f[d.cat].kind === "delta") {
          const held = deltas.find((x) => x.order === d.order) ??
            live.flatMap((x) => x._c7 ?? []).find((x) => x.order === d.order);
          if (!held) return;
          held.value += v.delta;
  ...
  const fireOff = (upto) => {
    ...
    const prefix = list.filter((e) => e.order <= upto && !removed.has(e.order))
      .filter((e) => _c5(e, _v, _ad, (x) => removed.has(x._order ?? x.order)))
      .map((e) => {
        const value = _w.get(e.order);
        return { ...e, _order: e.order, off: false,
          ...
          ...(_f[e.cat].timing === "oneShot" && _f[e.cat].kind === "set" && value !== undefined
            ? { op: "_w", value, readsCurrent: _E.includes(e.op) || e.readsCurrent } : {}) };
      });
    ...
    for (const i of rebuilt._v) _v.add(i);
  ...
  for (const e of list) {
    ...
    if (c.group === "original") {
      ...
      groupA.forEach((p) => { if (p.order < e.order) _v.add(p.order); });
    ...
    if (c.kind === "delta") {
      ...
      _bt();
      for (const item of live) {
        if (_v.has(item.eff.order)) continue;
    ...
    const _c7 = c.timing === "continuous" && readsCurrent ? [...deltas] : [];
    deltas.forEach((d) => {
      if (keep(d) || _c7.includes(d)) return;
      _v.add(d.order);
      _ad.set(d.order, e);
    });
    ...
    if (c.timing === "continuous" && _D.includes(e.op)) {
      live.push({ eff: e, lastInput: before, _c7,
...
export function _b(base, effects, other = [], otherBase = 0) {
  ...
  const without = (drop) => {
    const survivors = effects
      .map((e) => ({ e, i: e._order }))
      .filter(({ e, i }) => {
        return _c5(e, full._v, full._ad, drop);
      })
      .map(({ e, i }) =>
    ...
    <div className="yc-sub">
      ...
      {more && (
        <dl className="yc-terms">
          ...
          <dt>조건형 합산계열</dt>
          <dd>
            [고고고 골렘-GF]의 소재·릴리스나 [Em 햇 트리커]의 카운터처럼,
            조건을 만족했을 때 공격력을 한 번 정하고 끝나는 효과입니다. 비발동형 · 특정 수치 변경에서 고를 수 있고,
            계산된 수치를 직접 적어주세요.
            그 뒤 원래 공격력이 바뀌면 이 수치는 사라지고 돌아오지 않습니다.
          </dd>
          <dt>상대 수치</dt>
          <dd>
