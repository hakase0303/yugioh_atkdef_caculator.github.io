...
export function _a(base, effects) {
  ...
  for (const [i, e0] of _q.entries()) {
    const e = liveAtChange(e0, i) ? { ...e0, value: e0.vary.next } : e0;
...
export function _b(base, effects) {
  const full = _a(base, effects);

  
  const without = (drop) => {
    const _H = effects
      .map((e, i) => ({ e, i }))
      .filter(({ e, i }) => !drop(e) && (!full._v.has(i) || !!e.vary))
      .map(({ e, i }) =>
...
@container (max-width:560px){
  ...
  .yc-row[data-x="1"]{grid-template-areas:"n t f g m"
                                          "n v o o m"
                                          "n x . . m";}
  .yc-row[data-x="1"][data-num="0"]{grid-template-areas:"n t f g m"
                                                        "n o o o m"
                                                        "n x . . m";}
  ...
  .yc-row[data-x="1"][data-vary="1"][data-num="0"]{grid-template-areas:"n t f g m1"
                                                                       "n x y y y"
                                                                       "n o o o m2";}
  .yc-row[data-vary="1"] .yc-c-m{display:contents;}
  .yc-row[data-vary="1"] .yc-grip{grid-area:m1;}
  .yc-row[data-vary="1"] .yc-del{grid-area:m2;}
  .yc-row[data-vary="1"] .yc-grip,
  .yc-row[data-vary="1"] .yc-del{height:40px;align-self:center;}
  .yc-c-x{grid-area:x;font-size:10px;padding:0 2px;}
  .yc-chips{flex-direction:column;gap:4px;}
  .yc-chips .yc-chip{width:100%;padding:0;height:34px;font-size:10px;}
  
  .yc-row[data-both="1"] .yc-chips{height:100%;}
  .yc-row[data-both="1"] .yc-chips .yc-chip{flex:1;height:auto;}
  ...
  const _K = (s, e, i) => {
    ...
    return (
      <div
        ...
        data-both={canVary && e.varying && canOff && e.off ? "1" : "0"}
