...
const _o = ["up", "down", "fixed", "summonFixed"];

const _p = ["origDouble", "origTriple", "origHalf", "toOrig"];
const _c2 = ["foeAs", "foeHalf", "foeDouble", "foeTriple"];

const _D = ["double", "triple", "half", ..._p, ..._c2];

const _E = ["double", "triple", "half"];

const _d = (v) => Math.round(v);

const _C = (v) => Math.max(0, v);
...
function _c(value, eff, ref = value) {
  switch (eff.op) {
    ...
    case "frozen": return _d(eff.value);
    case "origDouble": return _d(ref * 2);
    case "origTriple": return _d(ref * 3);
    case "origHalf":   return _d(ref / 2);
    case "toOrig":     return _d(ref);
    case "foeAs":      return _d(eff.foe ?? 0);
    case "foeHalf":    return _d((eff.foe ?? 0) / 2);
    case "foeDouble":  return _d((eff.foe ?? 0) * 2);
    case "foeTriple":  return _d((eff.foe ?? 0) * 3);
...
const _e = (eff) => {
  ...
  if (_c2.includes(eff.op)) {
    const v = eff.foe ?? 0;
    return eff.op === "foeAs" ? `상대 수치 ${v} 가 된다`
      : eff.op === "foeHalf" ? `상대 수치 ${v} 의 절반`
      : eff.op === "foeDouble" ? `상대 수치 ${v} 의 2배`
      : `상대 수치 ${v} 의 3배`;
  }
  if (eff.summonFixed) return `${eff.value} 로 1회만 확정`;
  if (eff.op === "fixed" || eff.op === "frozen") return `${eff.value} 이 된다`;
  return o.label;
};

function _c5(e, wiped, _ad, drop) {
  if (drop(e)) return false;
  if (!wiped.has(e._order ?? e.order)) return true;
  if (e.vary && _f[e.cat].timing === "continuous") return true;
  const by = _ad.get(e._order ?? e.order);
  if (!by || !drop(by)) return false;
  if (_f[by.cat].timing === "oneShot") {
    return _f[e.cat].kind === "delta" &&
      (_f[e.cat].timing === "continuous" || by.wasSwap);
  }
  return _f[e.cat].kind === "delta" || _p.includes(e.op) ||
    _c2.includes(e.op) || _E.includes(e.op);
}

export function _a(base, effects, { flushOff = true, until = Infinity } = {}) {
  const list = effects.map((e, i) => ({ ...e, order: e._order ?? i }));
  const groupA = list.filter((e) => _f[e.cat].group === "original");
  const trace = [];
  ...
  const cur = () => {
    let v = anchor ? anchor.value : orig;
    for (const d of deltas) v = _d(v + d.value);
    if (live.some((x) => x.lockZero && !wiped.has(x.eff.order))) return 0;
    return v;
  };
  const underlying = (beneath, absorbed = []) => {
    let v = beneath ?? orig;
    for (const d of absorbed) v = _d(v + d.value);
    for (const d of deltas) v = _d(v + d.value);
    return v;
  };
  const inputOf = (e) => {
    if (_p.includes(e.op)) return orig;
    if (!anchor || anchor.order !== e.order) return cur();
    const item = live.find((x) => x.eff.order === e.order);
    return underlying(item?.beneath, item?.absorbed);
  };

  const push = (e, before, value, note) =>
    trace.push({ kind: "step", cat: e.cat, label: _e(e), before, value, note });

  const recalcLive = (skip, bySet, byEff) => {
    if (bySet && byEff && _p.includes(byEff.op)) return;
    const active = live.filter((x) => !x.lockZero && !wiped.has(x.eff.order));
    const chain = active.filter((x) => _E.includes(x.eff.op));
    if (chain.length > 1 && chain.length === active.length) {
      if (bySet && byEff?.cat === 4 && _E.includes(byEff.op)) return;
      const first = chain[0];
      if (bySet) {
        first.beneath = cur();
        first.beneathTiming = _f[byEff.cat].timing;
        first.beneathOrder = byEff.order;
        for (const item of chain) item.absorbed = [];
      }
      const held = new Map();
      for (const item of chain) {
        for (const d of item.absorbed ?? []) held.set(d.order, d);
        item.absorbed = [];
      }
      for (const d of deltas) held.set(d.order, d);
      first.absorbed = [...held.values()];
      let value = first.beneath ?? orig;
      for (const d of first.absorbed) value = _d(value + d.value);
      deltas = [];
      for (const item of chain) {
        const before = value;
        item.lastInput = value;
        value = _c(value, item.eff, orig);
        anchor = { value, timing: "continuous", order: item.eff.order, readsCurrent: true };
        push(item.eff, before, value, "재계산");
      }
      return;
    }
    for (const item of live) {
      if (item.lockZero) continue;
      if (wiped.has(item.eff.order) || item.eff.order === skip) continue;
      if (bySet && byEff?.cat === 4 && _E.includes(byEff.op) &&
          _E.includes(item.eff.op)) continue;
      const now = inputOf(item.eff);
      if (now === item.lastInput) continue;
      const before = cur();
      const readsCur = _E.includes(item.eff.op) || item.eff.readsCurrent;
      const own = anchor && anchor.order === item.eff.order;
      item.lastInput = now;
      anchor = { ...anchor, value: _c(now, item.eff, orig), order: item.eff.order };
      if (readsCur && own) {
        item.absorbed = [...(item.absorbed ?? []), ...deltas];
        deltas = [];
      } else if (readsCur) {
        item.beneath = now;
        item.absorbed = [];
  ...
  const fireVary = (upto) => {
    for (const d of list) {
      if (removed.has(d.order) || d.order > upto) continue;
      _bv(d).forEach((v, k) => {
        const key = `${d.order}:${k}`;
        if (fired.has(key) || v.after - 1 > upto) return;
        fired.add(key);
        if (wiped.has(d.order)) return;
        const before = cur();
        if (v.delta !== undefined && _f[d.cat].kind === "delta") {
          const held = deltas.find((x) => x.order === d.order) ??
            live.flatMap((x) => x.absorbed ?? []).find((x) => x.order === d.order);
          if (!held) return;
          held.value += v.delta;
        } else if (v.next !== undefined && _f[d.cat].group === "original") {
          orig = _c(orig, { ...d, value: v.next }, orig);
        } else if (v.next !== undefined && _f[d.cat].group === "current") {
          if (!anchor || anchor.order !== d.order) return;
          anchor = { ...anchor, value: _c(cur(), { ...d, value: v.next }, orig) };
        } else return;
        recalcLive();
        const shown = v.next ?? Math.abs(v.delta ?? 0);
        push(
          { ...d, op: v.delta !== undefined ? (v.delta < 0 ? "down" : "up") : d.op, value: shown }, before, cur(),
  ...
  const fireOff = (upto) => {
    const ending = list.filter((e) => e.off && !removed.has(e.order) &&
      e.order <= upto && Math.max(e.order + 1, Number(e.offAfter) || list.length) - 1 <= upto);
    if (!ending.length) return;
    ending.forEach((e) => removed.add(e.order));
    const prefix = list.filter((e) => e.order <= upto && !removed.has(e.order))
      .filter((e) => _c5(e, wiped, _ad, (x) => removed.has(x._order ?? x.order)))
      .map((e) => {
        const value = frozen.get(e.order);
        return { ...e, _order: e.order, off: false,
          vary: _bv(e).filter((v) => v.after - 1 <= upto),
          ...(_f[e.cat].timing === "oneShot" && _f[e.cat].kind === "set" && value !== undefined
            ? { op: "frozen", value, readsCurrent: _E.includes(e.op) || e.readsCurrent } : {}) };
      });
    const rebuilt = _a(base, prefix);
    ({ orig, anchor, deltas, live } = rebuilt._state);
    for (const item of live) item.eff = list.find((e) => e.order === item.eff.order) ?? item.eff;
    for (const i of rebuilt.wiped) wiped.add(i);
    for (const [i, by] of rebuilt._ad) _ad.set(i, by);
    ending.forEach((e) => trace.push({kind: "off", cat: e.cat, label: _e(e),
      value: cur(), note: "순서 " + (upto + 1) + " 이후 적용되지 않음"}));
  };
  ...
  for (const e of list) {
    ...
    if (c.group === "original") {
      ...
      const currentChain = live.filter((x) =>
        !wiped.has(x.eff.order) && _E.includes(x.eff.op));
      if (currentChain.length > 1 && currentChain[0].beneathTiming === "oneShot") {
        if (currentChain[0].beneathOrder !== undefined) wiped.add(currentChain[0].beneathOrder);
        currentChain[0].beneath = undefined;
        currentChain[0].beneathTiming = undefined;
        currentChain[0].beneathOrder = undefined;
      }
      groupA.forEach((p) => {
        if (p.order >= e.order) return;
        if (e.cat === 6 && p.cat === 6 &&
            (_E.includes(e.op) || _p.includes(e.op))) return;
        wiped.add(p.order);
      });
      if (anchor && anchor.timing === "oneShot") {
        wiped.add(anchor.order);
        anchor = null;
      }
      if (c.timing === "oneShot") frozen.set(e.order, orig);
      trace.push({
    ...
    if (c.kind === "delta") {
      const before = cur();
      deltas.push({
        order: e.order, timing: c.timing, temp: !!e.temp,
        value: e.op === "down" ? -e.value : e.value,
      });
      push(e, before, cur());
      recalcLive();
      for (const item of live) {
        if (wiped.has(item.eff.order)) continue;
        if (!_c2.includes(item.eff.op)) continue;
        const b3 = cur();
        anchor = { ...anchor, value: _c(orig, item.eff, orig),
                   order: item.eff.order };
        deltas = [];
        push(item.eff, b3, cur(), "재계산");
      }
      fireVary(e.order);
      fireOff(e.order);
      continue;
    ...
    const before = cur();
    const readAt = cur;
    const value = _c(readAt(), e, orig);
    const readsCurrent = _E.includes(e.op) || e.readsCurrent;

    const keep = (d) =>
      c.timing === "continuous" && !readsCurrent &&
      !_c2.includes(e.op) && d.timing === "continuous";
    const absorbed = c.timing === "continuous" && readsCurrent ? [...deltas] : [];
    deltas.forEach((d) => {
      if (keep(d) || absorbed.includes(d)) return;
      wiped.add(d.order);
      _ad.set(d.order, e);
    });
    deltas = deltas.filter(keep);
    const anchorWas = anchor;
    if (anchor && anchor.timing === "oneShot") {
      wiped.add(anchor.order);
      _ad.set(anchor.order, e);
    }

    anchor = { value, timing: c.timing, order: e.order, readsCurrent };
    if (e.cat === 4 && e.op === "fixed" && !e.wasSwap) {
      for (const item of live) {
        if (!item.lockZero || wiped.has(item.eff.order)) continue;
        wiped.add(item.eff.order);
        _ad.set(item.eff.order, e);
      }
    }
    if (c.timing === "oneShot") frozen.set(e.order, value);
    if (c.timing === "continuous" && _D.includes(e.op)) {
      live.push({ eff: e, lastInput: before, absorbed,
        beneath: anchorWas ? anchorWas.value : undefined,
        beneathTiming: anchorWas?.timing, beneathOrder: anchorWas?.order });
    }
    if (e.cat === 4 && e.op === "fixed" && e.value === 0 && !e.vary && !e.wasSwap) {
      live.push({ eff: e, lockZero: true });
    }
    push(e, before, cur());
    recalcLive(e.order, true, e);
    fireVary(e.order);
    fireOff(e.order);
  }

  fireVary(until);
  for (const at of [...new Set(list.filter((e) => flushOff && e.off && !removed.has(e.order))
    .map((e) => Math.max(e.order + 1, Number(e.offAfter) || list.length) - 1))].sort((a, b) => a - b)) {
    fireOff(at);
  }

  return {
    _state: { orig, anchor, deltas, live },
    _ad,
    original: _C(orig),
    current: _C(cur()),
    trace: trace.map((t) => ({
      ...t,
      value: _C(t.value),
      before: t.before === undefined ? undefined : _C(t.before),
    })),
    wiped, frozen,
  };
}
...
export function _b(base, effects, other = [], otherBase = 0) {
  const stamp = (list) => list.map((e, i) => ({ ...e, _order: e._order ?? i,
    offAfter: e.offAfter ?? list.length }));
  effects = stamp(effects);
  other = stamp(other);
  const resolve = (list, drop = () => false) =>
    _aa(list, other.filter((e) => !drop(e)), otherBase, base);

  const full = _a(base, resolve(effects));

  const without = (drop) => {
    const survivors = effects
      .map((e) => ({ e, i: e._order }))
      .filter(({ e, i }) => {
        return _c5(e, full.wiped, full._ad, drop);
      })
      .map(({ e, i }) =>
      ...
      .map(({ e, i }) => {
        const c = _f[e.cat];
        const value = full.frozen.get(i);
        const isOneShotSet = c.kind === "set" && c.timing === "oneShot";
        return isOneShotSet && value !== undefined
          ? { ...e, op: "frozen", value, readsCurrent: _E.includes(e.op) }
          : e;
      });
    return _a(base, resolve(survivors, drop));
  };

  const now = full;
  const after = without((e) => !!e.off || !!e.temp);
  const hasTemp = effects.some((e) => e.temp);
  return { now, after, hasTemp };
}

export function _aa(self, other, otherBase, selfBase = 0) {
  const a = self.map((e) => ({ ...e }));
  const b = other.map((e) => ({ ...e }));
  const ai = a.flatMap((e, i) => e.op === "swap" ? [i] : []);
  const bi = b.flatMap((e, i) => e.op === "swap" ? [i] : []);
  for (let k = 0; k < Math.max(ai.length, bi.length); k++) {
    const ia = ai[k], ib = bi[k];
    const ar = _a(Number(selfBase) || 0, a.slice(0, ia ?? a.length), { flushOff: false, until: ia === undefined ? Infinity : (a[ia]._order ?? ia) - 1 });
    const br = _a(Number(otherBase) || 0, b.slice(0, ib ?? b.length), { flushOff: false, until: ib === undefined ? Infinity : (b[ib]._order ?? ib) - 1 });
    const fixed = (e, r) => ({ ...e, op: "fixed",
      value: _f[e.cat].group === "original" ? r.original : r.current,
      readsCurrent: true, wasSwap: true });
    if (ia !== undefined) a[ia] = fixed(a[ia], br);
    if (ib !== undefined) b[ib] = fixed(b[ib], ar);
  }
  return a;
}
...
export const _ba = [
  ...
  { id: "summon", label: "조건형 합산계열", alone: true, onlyOff: true },
];
export const _bb = [
  { id: "as",     label: "가 된다" },
  { id: "half",   label: "절반" },
...
const _bc = {
  ...
  "foe|as": "foeAs",
  "foe|half": "foeHalf",
  "foe|double": "foeDouble",
  "foe|triple": "foeTriple",
  "plain|half": "half",
  "plain|double": "double",
  "plain|triple": "triple",
};
export const _bd = (base, mode) => {
  ...
  if (base === "summon") return "summonFixed";
...
const _d0 = () => ({ varying: false, count: "", count2: "", after: "1", steps: [] });
export function _c8(opBase) {
  const opMode = opBase === "fixed" ? "as" : "";
  return { opBase, opMode, op: _bd(opBase, opMode),
    ...(opBase === "summon" ? _d0() : {}) };
}
export function _c9(e, fire) {
  const invalidSummon = (e.opBase === "summon" || e.op === "summonFixed") &&
    !(fire === "off" && e.grp === "set");
  return { fire, ...(invalidSummon
    ? { op: null, opBase: "", opMode: "", summonFixed: false, ..._d0() } : {}) };
}

export function _c6(list, foe) {
  return list
        .map((e, i) => ({ ...e, _order: i }))
        .filter((e) => e.fire && e.grp && e.op)
        .filter((e) => e.op !== "summonFixed" || (e.fire === "off" && e.grp === "set"))
        .map((e) => (e.op === "zero" ? { ...e, op: "fixed", value: "0" } : e))
        .map((e) => (e.op === "summonFixed"
          ? { ...e, op: "fixed", fire: "on", summonFixed: true } : e))
        .filter((e) => !_o.includes(e.op) || String(e.value).trim() !== "")
        .map((e) => {
          const unit = Number(e.value) || 0;
          const base = {
            _order: e._order, cat: _k(e.grp, e.fire), op: e.op,
            value: unit, temp: !!e.temp, off: !!e.off, offAfter: Number(e.offAfter) || 1,
            foe, summonFixed: !!e.summonFixed,
          };
          if (!e.varying || e.summonFixed) return base;
          const counts = [
            Number(e.count) || 0,
            Number(e.count2) || 0,
          ...
          const vary = afters.map((after, k) => ({
            ...
            ...(e.grp === "delta"
              ? { delta: (e.op === "down" ? -1 : 1) * unit * (counts[k + 1] - counts[k]) }
      ...
      {more && (
        <dl className="yc-terms">
          ...
          <dd>
            ...
            비발동형에서는 “동안”으로 바뀌며,
            [수호신관 마하드]처럼 “~하는 동안”에만 적용되는 효과에 사용합니다.
            이 경우 그 사이가 끝난 뒤의 수치도 함께 볼 수 있습니다.
          </dd>
          <dt>카드수</dt>
          <dd>
          ...
          <dt>조건형 합산계열</dt>
          <dd>
            [고고고 골렘-GF]나 [Em 햇트리커]처럼 소환하거나 특정 조건을 충족했을 때
            공수 수치를 한 번 정하는 비발동 효과입니다. 비발동형 · 특정 수치 변경에서
            고를 수 있고, 효과로 정해진 최종 수치를 직접 적어주세요.
            [Em 햇트리커]는 카운터가 3개가 되어 정해지는 3300을 입력합니다.
            그 뒤 원래 공격력이 바뀌면 이 수치는 사라집니다.
          </dd>
          <dt>상대 수치</dt>
          <dd>
            [클리어 바이스 드래곤]을 위한 칸입니다.
            착한 듀얼리스트들은 쓸 일이 없습니다.
          </dd>
        </dl>
      )}
    </div>
...
.yc-theme{flex:0 0 auto;height:36px;min-width:36px;padding:0 9px;border:1px solid var(--edge);
  border-radius:9px;background:var(--btn);color:var(--btnInk);cursor:pointer;
...
.yc-in{padding:18px 4px 12px;position:relative;}
.yc-reset{position:absolute;top:10px;right:12px;width:28px;height:28px;
  display:flex;align-items:center;justify-content:center;
  background:var(--btn);border:1px solid var(--edge);border-radius:50%;
  color:var(--btnInk);cursor:pointer;font:inherit;font-size:15px;line-height:1;padding:0;}
.yc-reset:hover{border-color:var(--gold);color:var(--gold);}
.yc-in > .yc-lab,.yc-in > .yc-input{margin-left:12px;margin-right:12px;width:auto;}
.yc-in > .yc-baserow{margin-left:12px;margin-right:12px;}
.yc-baserow{display:flex;justify-content:space-between;align-items:flex-end;gap:12px;}
.yc-baserow .yc-lab{margin:0 0 6px;}
.yc-basefield{flex:0 0 auto;width:200px;}
.yc-basefield .yc-input{width:100%;margin:0;}
.yc-oppo{width:160px;margin-right:34px;}
.yc-oppo .yc-lab{text-align:right;}
...
@container (min-width:561px){
  ...
  .yc-var{grid-column:2/-3;grid-row:2;display:flex;flex-wrap:nowrap;align-items:flex-end;gap:6px;
    margin:2px 0 4px;padding:0;font-size:11.5px;color:var(--muted);}
  .yc-off{grid-column:2/-3;grid-row:2;display:flex;flex-wrap:nowrap;
    align-items:flex-end;gap:6px;
    margin:2px 0 4px;padding:0;font-size:11.5px;color:var(--muted);}
  .yc-row[data-vary="1"] .yc-off{grid-column:-3/-1;}
  .yc-row > .yc-off > .yc-varf{flex:0 0 125px;width:125px;min-width:0;}
...
@container (max-width:560px){
  ...
  .yc-in > .yc-baserow{margin-left:6px;margin-right:6px;}
  .yc-baserow{display:grid;grid-template-columns:2fr 1fr;gap:8px;align-items:end;}
  .yc-basefield{width:auto;}
  .yc-oppo{width:auto;margin-right:0;grid-column:2;grid-row:1;margin-top:24px;}
  .yc-oppo .yc-lab{text-align:left;}
  .yc-baserow .yc-lab{margin-left:0;margin-right:0;}
  .yc-baserow .yc-input{width:100%;}
...
export default function StatusCalculator() {
  ...
  const [_c3, _c4] = useState({ 공격력: "", 수비력: "" });
  ...
  const results = useMemo(() => {
    const prepped = Object.fromEntries(
      STATS.map((s) => [s, _c6(lists[s], Number(_c3[s]) || 0)])
    );
    ...
    return Object.fromEntries(
      STATS.map((s) => {
        ...
        return [
          s,
          _b(
            Number(bases[s]) || 0, prepped[s],
            prepped[otherName], Number(bases[otherName]) || 0
          ),
        ];
      })
    );
  ...
  const _bq = (s, e, opBase, opMode) => {
    ...
    if (isSwap) {
      setList(other, (l) => [
        ...l,
        { ...newEffect(), twin: e.id, fire: e.fire, grp: e.grp, temp: e.temp,
          opBase: "swap", opMode: "", op: "swap" },
      ]);
    } else {
      setList(other, (l) => l.filter((x) => x.twin !== e.id));
    }
  };
  ...
  const _br = (s, e) => {
    setList(s, (l) => l.filter((x) => x.id !== e.id));
    setList(_bs(s), (l) => l.filter((x) => x.twin !== e.id && x.id !== e.twin));
  };
  ...
  const effectRow = (s, e, i) => {
    ...
    const needsNum = e.grp === "delta" || e.op === "fixed" || e.op === "summonFixed";
    const needsMode =
      kind === "set" && !!e.opBase &&
            ...
            <>
              <select
                ...
                onChange={(ev) => {
                  ...
                  _bq(s, e, opBase);
                  patch(s, e.id, _c8(opBase));
                }}
              >
                <option value="" disabled>선택</option>
                {_ba.filter(
                  (o) => (o.id !== "foe" || e.grp !== "orig") &&
                         (!o.onlyOff || (e.fire === "off" && e.grp === "set"))
                ).map((o) => (
  ...
  const _bz = (s) => {
    setBases((b) => ({ ...b, [s]: "" }));
    const ids = lists[s].map((e) => e.id);
    setList(s, () => []);
    setList(_bs(s), (l) =>
      l.filter((x) => !ids.includes(x.twin) && !ids.includes(x.id))
    );
  };

  const inputPanel = (s) => (
    <div className="yc-panel yc-in">
      <button className="yc-reset" title="처음 상태로 되돌리기"
        aria-label={`${s} 초기화`} onClick={() => _bz(s)}>↺</button>
      <div className="yc-baserow">
        <div className="yc-basefield">
        ...
        {lists[s].some((e) => e.opBase === "foe") && (
          <div className="yc-basefield yc-oppo">
            <label className="yc-lab" htmlFor={`yc-oppo-${s}`}>상대 수치</label>
            <input
              id={`yc-oppo-${s}`} className="yc-input" inputMode="numeric"
              value={_c3[s]}
              onChange={(ev) => _c4((o) => ({ ...o, [s]: ev.target.value }))}
            />
          </div>
        )}
      </div>
